...
const Header = () => {
  ...
  const isModalOpen = useViewStore((state) => state.isModalOpen);
  const setIsModalOpen = useViewStore((state) => state.setIsModalOpen);
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shadow-sm z-20 shrink-0">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Add Button */}
        <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg shadow-blue-500/30 transition-all active:scale-95 flex items-center gap-2">
          <svg
            className="w-4 h-4"
            fill="none"
            stroke="currentColor"
